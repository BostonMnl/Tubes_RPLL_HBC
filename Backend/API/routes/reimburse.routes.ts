...
router.post('/for-user', authMiddleware(['admin'], ['manager', 'supervisor']), uploadImage.single('gambar'), apiResponse(createReimburseRequestForUser));
router.get('/me', authMiddleware(), apiResponse(getMyReimburse));
router.get('/', authMiddleware(['admin'], ['manager', 'supervisor']), apiResponse(getAllReimburseRequests));
...
router.put('/:id/approval', authMiddleware(['admin'], ['manager', 'supervisor']), apiResponse(approveDeclineReimburseRequest));
